...
| `configure_device_ble` | enabled: true/false | 开关 BLE 蓝牙服务 |
| `set_ble_discoverable` | enabled: true/false | BLE 发现模式 (120s 自动关闭) |
...
|------|------|---------|
| WiFi TCP | <10ms | 首选，自动 |
| BLE | 0.5-2s | WiFi 不可用时自动回退（需先配对绑定） |
...
| 工具 | 机制 | 配置位置 |
|------|------|---------|
| MiMoCode | JS 插件 | `.mimocode/plugins/esp32-led.js` |
| OpenCode | JS 插件 | `.opencode/plugins/esp32-led.js` |
...
python transport/send.py --bind esp32-led-7604F1A8                # 绑定设备
```
