...
| 触发条件 | 调用 | 时机 |
|---------|------|------|
| 收到用户消息、开始处理请求 | `send_led_state(state="thinking", platform="xxx", project="当前项目名")` | 第一动作，在任何思考/回答之前 |
| 执行 `Edit` / `Write` / 任何写入文件的操作 | `send_led_state(state="coding", platform="xxx", project="当前项目名")` | 写入前 |
| 执行 `Bash` / 终端命令 | `send_led_state(state="busy", platform="xxx", project="当前项目名")` | 命令执行前 |
| 需要用户确认权限 / 等待用户输入 | `send_led_state(state="waiting", platform="xxx", project="当前项目名")` | 等待前 |
| 用户做出选择或回复后 | `send_led_state(state="thinking", platform="xxx", project="当前项目名")` | 收到回复后立即 |
| 任务成功完成 | `send_led_state(state="success", platform="xxx", project="当前项目名")` | 完成时 |
| 命令执行失败或出错 | `send_led_state(state="error", platform="xxx", project="当前项目名")` | 失败后立即 |
| 遇到 API 错误、异常、崩溃 | `send_led_state(state="alarm", platform="xxx", project="当前项目名")` | 异常后立即 |
| 会话空闲 / 任务结束 | `send_led_state(state="off", platform="xxx", project="当前项目名")` | 最终动作 |
